...
//       获取上一级界面（StarFragment）传过来的数据
        Intent intent = getIntent();
        bean = (StarBean.StarinfoBean) intent.getSerializableExtra("star");

        iinitView();
        mDatas = new ArrayList<>();//  初始化显示 在ListView上的数据源
        adapter = new AnalysisBaseAdapter(this, mDatas);
        analysisLv.setAdapter(adapter); // 此时是没有数据的
        addDataToList();
    }
...
//        将数据进行显示
        ...
        /**
         * 以下数据不考虑ListView，不用考虑适配器的，直接进行设置的
         */
