...
//        获取LuckFragment传递过来的参数
        Intent intent = getIntent();
//        获取上一级界面传递的星座名称
        String name = intent.getStringExtra("name");
        System.out.println("LuckAnalysisActivity  ====>>>>> name ======>>>>>>>>"+name);

//        获取网址
        String luckURL = URLContent.getLuckURL(name);
        System.out.println("LuckAnalysisActivity  ====>>>>> luckURL ===="+luckURL);

        //        查找控件
        initView(name);
        mDatas = new ArrayList<>(); // 初始化集合
...
//        获取网络请求
    ...
    private void initView(String name) {
        System.out.println("LuckAnalysisActivity  >>>>>>>>>>>>  initView");
        //        查找控件
...
//            数据的解析
            ...
            System.out.println("运势2222："+luckBean);
//            为了显示在ListView上，重新整理数据 ，整理成集合形式
            addDataToList(luckBean);
            System.out.println("整理的数据 ："+mDatas);
//            设置适配器
    ...
    private void addDataToList(LuckBean luckBean) {
        System.out.println("LuckAnalysisActivity >>>>>>>>>>>>>>>>  addDataToList ");
